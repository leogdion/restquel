  ...
  restquel.prototype = {
    ...
    callback: function (request, response, next) {
      ...
      if (key && this.__queries[key]) {
        ...
        this.__queries[key](require.query || require('url').parse(request.url).query, this.respond.bind(this, key, response));
      } else if (next) {
        next();
      } else if (this.__next) {
        this.__next();
    ...
    respond: function (key, response, error, results) {
  ...
  restquel.fromConfiguration = function (configuration, next) {
    if (typeof configuration === "string") {
      ...
      for (var key in configuration.queries) {
        ...
        configuration.queries[key].text = (Array.isArray(text) && text.join('')) || (fs.existsSync(textPath) && fs.readFileSync(textPath).toString()) || text;
        var text = configuration.queries[key].text;
      }
    }
